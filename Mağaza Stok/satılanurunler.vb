...
Imports System.IO
Public Class satılanurunler
    Private Sub btnkayit_Click(sender As Object, e As EventArgs) Handles btnkayit.Click
        anasayfa.Show()
        Me.Hide()
    End Sub

    Private Sub satılanurunler_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub ExportDataToPDFTable()
        ...
        Dim widths As Single() = New Single() {2.0F, 2.0F, 2.0F, 2.0F, 2.0F, 2.0F, 2.0F}
        ...
        PdfPCell.HorizontalAlignment = PdfPCell.ALIGN_CENTER
        PdfTable.AddCell(PdfPCell)
        PdfPCell = New PdfPCell(New Phrase(New Chunk("Urun Fiyati", font12Bold)))
        PdfPCell.HorizontalAlignment = PdfPCell.ALIGN_CENTER
        PdfTable.AddCell(PdfPCell)
        PdfPCell = New PdfPCell(New Phrase(New Chunk("Urun KDV", font12Bold)))
        PdfPCell.HorizontalAlignment = PdfPCell.ALIGN_CENTER
        PdfTable.AddCell(PdfPCell)
        PdfPCell = New PdfPCell(New Phrase(New Chunk("Genel Toplam", font12Bold)))
        PdfPCell.HorizontalAlignment = PdfPCell.ALIGN_CENTER
        PdfTable.AddCell(PdfPCell)
        PdfPCell = New PdfPCell(New Phrase(New Chunk("Tarih", font12Bold)))
        PdfPCell.HorizontalAlignment = PdfPCell.ALIGN_CENTER
        PdfTable.AddCell(PdfPCell)
        PdfPCell = New PdfPCell(New Phrase(New Chunk("Urun Barkodu", font12Bold)))
        PdfPCell.HorizontalAlignment = PdfPCell.ALIGN_CENTER
        PdfTable.AddCell(PdfPCell)
    ...
    Private Sub tarih_ValueChanged(sender As Object, e As EventArgs) Handles tarih.ValueChanged
        Using baglanti As New SQLiteConnection("data source=db/magaza.db")
            Using getir As New SQLiteCommand($"select * from satilan_urunler where tarih='{tarih.Value.ToString("dd/MM/yyyy")}'", baglanti)
                baglanti.Open()
